...
namespace Infraestructure.Query
{
    public class SaleQuery : ISaleQuery
    {
        ...
        public async Task<List<Sale>> GetAllSale()
        {
            try
            {
                List<Sale> allSale = _context.Sale.Include(s => s.SaleMotos).ToList();
                return allSale;
            }
        ...
        public async Task<List<Sale>> GetSaleByDate(DateTime fromDate, DateTime toDate)
        {
            try
            {
                List<Sale> salebydate = _context.Sale.Include(s => s.SaleMotos).Where(s => s.Date.Date >= fromDate.Date && s.Date.Date <= toDate.Date).ToList();
                return salebydate;
            }
            catch (DbUpdateException)
            {
        ...
        public async Task<Sale> GetSaleById(int saleId)
        {
            try
            {
                return await _context.Sale.Include(s => s.SaleMotos).SingleOrDefaultAsync(s => s.SaleId == saleId);
            }
